...
const Todo = () => {
    ...
    function addToDoItem(){
        ...
        if(todo_list.length){
            setTodo((ele)=>([...ele,{id:todo.length+1,todo:todo_list,ischecked:false}]));
  ...
  return (
    <div>
        ...
        <div id ="todoConatiner"  className="todoContainer position-relative">
            ...
            <input className="form-check-input mx-3 " type="checkbox" onClick={(e)=> handle_check(e,ele.id)}></input>{(edit==ele.id) ? (<input type='text' value={edit_input} onChange={(e)=> setEdit_input(e.target.value)}></input>,<button
            className="btn btn-outline-success rounded p-2 ms-2">Save</button> )  :  <span className={(ele.ischecked) ? "text-decoration-line-through"  : ""}>{ele.todo}</span> }
            <div>
